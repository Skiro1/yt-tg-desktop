...
from PyQt6.QtWidgets import (
    ...
    QRadioButton, QCheckBox, QStatusBar,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.tg_client = None
        self._caption_user_modified = False
    ...
    def check_credentials(self):
        ...
        if not api_id or not api_hash:
            self.status_bar.showMessage("Settings not configured. Open Settings to enter API credentials.")
            return
        ...
        try:
            ...
            self.status_bar.showMessage(f"Configuration Error: {str(e)}")
    ...
    def init_ui(self):
        ...
        tg_layout.addWidget(self.btn_settings)
        ...
        # Группа: только 1 тег за раз (ручная реализация для возможности снятия)
        self.chk_gp.clicked.connect(self._on_tag_clicked)
        self.chk_deco.clicked.connect(self._on_tag_clicked)
        self.chk_gp_deco.clicked.connect(self._on_tag_clicked)
        self.chk_preview.clicked.connect(self._on_tag_clicked)
    ...
    def _on_caption_edited(self):
        self._caption_user_modified = True
    ...
    def on_info_received(self, info: dict):
        ...
        # Сбрасываем флаг ручного редактирования перед генерацией
        self._caption_user_modified = False
    ...
    def on_caption_type_changed(self):
        ...
        self.update_caption_display(from_tag=False)

    def _on_tag_clicked(self):
        sender = self.sender()
        if sender.isChecked():
            # Выбран новый тег — снимаем остальные
            for btn in (self.chk_gp, self.chk_deco, self.chk_gp_deco, self.chk_preview):
                if btn != sender:
                    btn.setChecked(False)
        # Если кликнули уже выбранный тег, Qt его снял — просто обновляем caption
        self.update_caption_display(from_tag=True)

    def update_caption_display(self, from_tag=False):
        """Обновляет текст в caption_edit на основе выбранных опций."""
        if not self.current_info:
            return

        # Собираем текущие теги
        tags = []
        ...
        if self._caption_user_modified and not from_tag:
            # Только смена формата (с/без описания) не трогает ручной текст
            return

        if self._caption_user_modified and from_tag:
            # Пользователь редактировал текст, но поменял тег:
            # убираем старые теги из текущего текста и добавляем новые
            caption = self.caption_edit.toPlainText()
            caption = remove_all_tags(caption)
            for tag in tags:
                caption = add_tag_to_caption(caption, tag)
        else:
            # Обычный путь: регенерируем из базового caption
        ...
        # Блокируем сигнал, чтобы не ставить _caption_user_modified
